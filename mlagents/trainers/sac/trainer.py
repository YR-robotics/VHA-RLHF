...
from mlagents.trainers.policy.torch_policy_reward_model import TorchPolicy_R
...
class SACTrainer(OffPolicyTrainer):
    ...
    def create_policy(
        ...
        """
        创建一个使用 PyTorch 的 SAC 策略。
        :param parsed_behavior_id: 经过解析的行为 ID
        :param behavior_spec: 策略构建的规范
        :return policy: TorchPolicy 类的实例
        """
        actor_cls = SimpleActor
        actor_kwargs = {"conditional_sigma": True, "tanh_squash": True}
        policy = TorchPolicy_R(
            self.seed,
            behavior_spec,
    ...
    def get_policy(self, name_behavior_id: str) -> Policy:
        """
        从训练器中获取与 name_behavior_id 相关的策略
        :param name_behavior_id: 策略的完整标识符
        """
        return self.policy
    @staticmethod
