...
class AgentBuffer(MutableMapping):
    ...
    def make_mini_batch(self, start: int, end: int) -> "AgentBuffer":
        ...
        return mini_batch
    def idx_mini_batch(self, idx, tail) -> "AgentBuffer":
        """
        Creates a mini-batch from buffer.
        :param start: Starting index of buffer.
        :param end: Ending index of buffer.
        ...
        """
        mini_batch = AgentBuffer()
        tail_buffer = self.make_mini_batch(tail-1, -1)
        for key, field in tail_buffer._fields.items():
            # slicing AgentBufferField returns a List[Any}
            # print("len(field)", len(field))
            # print("idx", idx)
            mini_batch[key] = [field[i] for i in idx]  # type: ignore
        return mini_batch
    ...
    def num_experiences(self) -> int:
        ...
        if self.values():
            ...
            return 0
    def relabel_with_predictor(self, predictor, reward_signals, obses_ori, actions_ori):
        batch_size = 200
        total_iter = int(len(obses_ori)/batch_size)
        # n_obs = len(self.policy.behavior_spec.observation_specs)
        # current_obs = ObsUtil.from_buffer(self, n_obs)
        # current_obs2 = current_obs
        # # Convert to tensors
        # current_obs = [ModelUtils.list_to_tensor(obs) for obs in current_obs]
        # dones2 = self[BufferKey.DONE]
        # next_obs = ObsUtil.from_buffer_next(self, n_obs)
        # # Convert to tensors
        # next_obs2 = next_obs
        # next_obs = [ModelUtils.list_to_tensor(obs) for obs in next_obs]
        # actions = AgentAction.from_buffer(self)
        # cont_actions = actions.continuous_tensor
        # print("len(obses_ori):", len(obses_ori))
        if len(obses_ori) > batch_size*total_iter:
            total_iter += 1
        # obses_ori = np.array(obses_ori)
        # actions_ori = np.array(actions_ori)
        obses_ori = np.array(obses_ori)
        actions_ori = np.array(actions_ori)
        # print("obses,",len(obses_ori))
        # print("actions,",len(actions_ori))
        for index in range(total_iter):
            last_index = (index+1)*batch_size
            if (index+1)*batch_size > len(obses_ori):
                last_index = len(obses_ori)
            obses = obses_ori[index*batch_size:last_index]
            actions = actions_ori[index*batch_size:last_index]
            # print("actions:", type(actions[0]))
            inputs = np.concatenate([obses, actions], axis=-1)
            # inputs = torch.cat((obses, actions), axis=-1)
            pred_reward = predictor.r_hat_batch(inputs)
            # self.rewards[index*batch_size:last_index] = pred_reward
            for name, signal in reward_signals.items():
                self[RewardSignalUtil.rewards_key(name)][index*batch_size:last_index] = pred_reward.flatten()
                # print(self[RewardSignalUtil.rewards_key(name)][index*batch_size:last_index])
