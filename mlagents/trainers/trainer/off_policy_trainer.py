...
from mlagents.trainers.buffer import RewardSignalUtil, ObservationKeyPrefix
        ...
        while (
            ...
            if self.update_buffer.num_experiences >= self.hyperparameters.batch_size:
                ...
                for name, signal in self.optimizer.reward_signals.items():
                    ...
                    rewards = sampled_minibatch[RewardSignalUtil.rewards_key(name)]
                # obs_list = []
                # for sampled_mini in sampled_minibatch:
                #     if type(sampled_mini) == tuple:
                #         if sampled_mini[0] == ObservationKeyPrefix.OBSERVATION:
                #             for sampled_mini2 in sampled_minibatch[sampled_mini]:
                #                 print("len", len(sampled_minibatch[sampled_mini][0]))
                #                 obs_list.append(sampled_mini2)
                # print("obs_list:", obs_list[1])
                update_stats, current_obs, next_obs, actions_, rewards_, dones = self.optimizer.update(sampled_minibatch, n_sequences)
                # obs_all = []
                ...
                # actions = sampled_minibatch[BufferKey.CONTINUOUS_ACTION]
                ...
                # actions_all = []
                # for j in range(0, len(actions[0])):
                #     actions_all.append([])
                #     for i in range(0, len(actions)):
                #         actions_all[j].extend(actions[i][j])
                # print("dones:", dones)
