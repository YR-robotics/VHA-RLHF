...
class RLTrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.update_buffer_all: AgentBuffer = AgentBuffer()
    ...
    def _append_to_update_buffer_all(self, agentbuffer_trajectory: AgentBuffer) -> None:
        """
        Append an AgentBuffer to the update buffer. If the trainer isn't training,
        don't update to avoid a memory leak.
        """
        if self.should_still_train:
            ...
            agentbuffer_trajectory.resequence_and_append(
                self.update_buffer_all, training_length=seq_len
            )
