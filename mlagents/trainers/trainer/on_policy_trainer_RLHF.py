...
class OnPolicyTrainer(RLTrainer):
    ...
    def _update_policy_R(self):
        ...
        num_epoch = self.hyperparameters.num_epoch
        batch_update_stats = defaultdict(list)
        if self._step <= Cfg.NUM_UNSUP_STEPS.value:
            print("unsup")
            self._append_to_update_buffer_all(self.update_buffer)
            update_stats = self.optimizer.update_state_ent(batch_all=self.update_buffer_all, batch_size=1024, gradient_update=1, K=5, normalize_state_entropy=True, tail_=tail)
        elif self._step > Cfg.NUM_UNSUP_STEPS.value:       
            for _ in range(num_epoch):
                self.update_buffer.shuffle(sequence_length=self.policy.sequence_length)
                buffer = self.update_buffer
                max_num_batch = buffer_length // batch_size
